...
def plotly_table(dataframe):
    ...
    fig = go.Figure(data=[go.Table(
        ...
        cells=dict(
            ...
            fill_color=[
                [rowOddColor, rowEvenColor] * (len(dataframe)//2 + 1)
...
def RSI(dataframe, num_period):
    dataframe['RSI'] = ta.momentum.RSIIndicator(dataframe['Close'], 14).rsi()
    dataframe = filter_data(dataframe, num_period)
    fig = go.Figure()
...
def MACD(dataframe, num_period):
    ...
    colors = ['green' if v > 0 else 'red' for v in dataframe['MACD Hist']]
    fig.add_trace(go.Bar(x=dataframe['Date'], y=dataframe['MACD Hist'],
                         name='Histogram', marker_color=colors))

    fig.update_layout(height=300, plot_bgcolor='white', paper_bgcolor='#e1efff',
                      margin=dict(l=0, r=0, t=0, b=0),
                      legend=dict(orientation="h", yanchor="top", y=1.02, xanchor="right", x=1))
    return fig
