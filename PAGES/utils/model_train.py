...
def fit_model(data, differencing_order):
    series = data.squeeze() if isinstance(data, pd.DataFrame) else data
    model = ARIMA(series, order=(30, differencing_order, 30))
    model_fit = model.fit()
    forecast_steps = 30
...
def evalute_mode(original_price, differencing_order):
    series = original_price.squeeze() if isinstance(original_price, pd.DataFrame) else original_price
    train_data, test_data = series[:-30], series[-30:]
...
def get_forecast(original_price, differencing_order):
    series = original_price.squeeze() if isinstance(original_price, pd.DataFrame) else original_price
    predictions = fit_model(series, differencing_order)
